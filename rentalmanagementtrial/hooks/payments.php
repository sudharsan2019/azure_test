<?php
	...
	function payments_before_insert(&$data, $memberInfo, &$args){
		...
		elseif ($count>0) {
			# code...
			$_SESSION['custom_err_msg']="<b>Sorry!! The User Invoice ID seems to be already on track,You can only add payments for invoices that are not on track.If its a subsequent payment kindly find the invoice id and update the payments accordingly!!</b>";
			return FALSE;
		}
		else{
			return TRUE;
	...
	function payments_after_insert($data, $memberInfo, &$args){
		...
		sql("update payments set balance='$getbalance' where id='$currid'" ,$eo);
